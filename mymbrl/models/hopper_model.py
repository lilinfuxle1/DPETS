...
class HopperModel(nn.Module):
    ...
    def __init__(self, ensemble_size, in_features, out_features,
                 hidden_size=200, drop_prob=0.1, dropout_mask_nums=20, device="cpu"):
        super().__init__()
        ...
        # affine parameters for each layer
        ...
        self.lin2_w, self.lin2_b = get_affine_params(ensemble_size, hidden_size, out_features)
        ...
        self.min_logvar = nn.Parameter(- torch.ones(1, out_features // 2, dtype=torch.float32).to(device) * 3.0)
    ...
    def forward(self, inputs, ret_logvar=False, open_dropout=True):
        ...
        inputs = swish(inputs.matmul(self.lin0_w) + self.lin0_b)
        if self.dropout and open_dropout:
            inputs = inputs * self.hidden1_mask_select

        inputs = swish(inputs.matmul(self.lin1_w) + self.lin1_b)
        if self.dropout and open_dropout:
